...
def _evento(perfil, classe=None, dur_h=2, **kw):
    return EventoFactory(
        dono=perfil,
        classe=classe or ClasseFactory(dono=perfil),
...
def test_concluir_sem_real_min_grava_registro(perfil):
    ev = _evento(perfil, dur_h=2)
    completion.concluir(ev)
    r = RegistroExecucao.objects.do_dono(perfil).get()
...
@pytest.mark.django_db
def test_concluir_com_real_min_via_endpoint(api, perfil):
    ev = _evento(perfil, dur_h=2)
    resp = api.post(
        f"/api/v1/eventos/{ev.id}/concluir/", {"real_min": 90}, format="json"
    )
    assert resp.status_code == 200
    assert RegistroExecucao.objects.do_dono(perfil).get().real_min == 90


@pytest.mark.django_db
def test_concluir_real_min_invalido_400(api, perfil):
    ev = _evento(perfil)
    for ruim in ("x", 0, -5):
        resp = api.post(
...
@pytest.mark.django_db
def test_remarcar_grava_registro_remarcado(perfil):
    tarefa = TarefaFactory(dono=perfil)
    ev = _evento(perfil, origem_tarefa=tarefa)
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_fator_neutro_com_poucas_amostras(perfil):
    classe = ClasseFactory(dono=perfil)
    _registro(classe, 60, 78)
    _registro(classe, 60, 78)
    assert adaptacao.fator_classe(perfil, str(classe.id)) == 1.0  # < 3 amostras


@pytest.mark.django_db
def test_fator_converge_para_a_razao_real(perfil):
    classe = ClasseFactory(dono=perfil)
    for _ in range(10):
    ...
    assert adaptacao.fator_classe(perfil, str(classe.id)) == round(esperado, 2)
    assert adaptacao.fator_classe(perfil, str(classe.id)) > 1.25  # convergiu p/ ~1.3


@pytest.mark.django_db
def test_fator_respeita_clamp(perfil):
    lento = ClasseFactory(dono=perfil)
    for _ in range(20):
        _registro(lento, 10, 300)  # razão 30 → clampa em 3.0
    assert adaptacao.fator_classe(perfil, str(lento.id)) == adaptacao.FATOR_MAXIMO

    rapido = ClasseFactory(dono=perfil)
    for _ in range(20):
        _registro(rapido, 300, 10)  # razão ~0.03 → clampa em 0.5
    assert adaptacao.fator_classe(perfil, str(rapido.id)) == adaptacao.FATOR_MINIMO


@pytest.mark.django_db
def test_fator_ignora_registros_sem_real_min_e_ids_invalidos(perfil):
    classe = ClasseFactory(dono=perfil)
    for _ in range(5):
        _registro(classe, 60, None)  # sem real_min: vale só p/ flexibilidade
    assert adaptacao.fator_classe(perfil, str(classe.id)) == 1.0
    assert adaptacao.fator_classe(perfil, "c1") == 1.0  # id hipotético (não-UUID)
    assert adaptacao.fator_classe(perfil, None) == 1.0
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_flexibilidade_e_a_taxa_de_remarcacao(perfil):
    classe = ClasseFactory(dono=perfil)
    _registro(classe, remarcado=True)
    _registro(classe, remarcado=True)
    _registro(classe, remarcado=False)
    _registro(classe, remarcado=False)
    assert adaptacao.flexibilidade_classe(perfil, str(classe.id)) == 0.5
    assert (
        adaptacao.flexibilidade_classe(perfil, str(ClasseFactory(dono=perfil).id))
        == 0.0
    )
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_decaimento_move_pesos_rumo_ao_neutro(perfil):
    PesoPreferencia.objects.create(dono=perfil, metrica="fds_livres", valor=2.0)
    PesoPreferencia.objects.create(dono=perfil, metrica="pico_min_dia", valor=0.5)
    novos = adaptacao.decair_pesos(perfil)
    ...
    assert (
        not PesoPreferencia.objects.do_dono(perfil)
        .filter(metrica="dias_livres")
        .exists()
    )
...
# --------------------------------------------------------------------------- #
@pytest.mark.django_db
def test_solver_aloca_a_mais_com_fator_e_expoe_no_echo(perfil):
    classe = ClasseFactory(dono=perfil)
    for _ in range(20):
        _registro(classe, 60, 78)  # fator → 1.3
    fator = adaptacao.fator_classe(perfil, str(classe.id))
    tarefa = TarefaFactory(
        dono=perfil, classe=classe, esforco_estimado=100, deadline=aware(2026, 6, 5, 18)
    )

    res = P.montar_plano(perfil, [tarefa], SEG, {})
    ...
    res = P.montar_plano(perfil, [tarefa], SEG, {}, usar_fatores=False)
    assert sum(s.dur_min for s in res.sessoes) == 100
    assert "fatores_classe" not in res.prefs_usadas


@pytest.mark.django_db
def test_contexto_contem_fatos_adaptativos(perfil):
    classe = ClasseFactory(dono=perfil)
    for _ in range(4):
        _registro(classe, 60, 90, remarcado=False)
    _registro(classe, remarcado=True)
    tarefa = TarefaFactory(
        dono=perfil, classe=classe, esforco_estimado=60, deadline=aware(2026, 6, 5, 18)
    )
    res = P.montar_plano(perfil, [tarefa], SEG, {})
    ctx = IA.construir_contexto(res)

    cid = str(classe.id)
    assert ctx["fatores_classe"][cid] == adaptacao.fator_classe(perfil, cid)
    assert ctx["flexibilidade_classe"][cid] == adaptacao.flexibilidade_classe(
        perfil, cid
    )
